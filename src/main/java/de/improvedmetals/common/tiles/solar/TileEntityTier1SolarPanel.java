...
public class TileEntityTier1SolarPanel extends TileEntity implements ITickable{

	 private final ContainerSolarPanelTier1 container;
	    
	    public TileEntityTier1SolarPanel() {
	        
	        this.container = new ContainerSolarPanelTier1();
	    }
	        ...
	        if (this.hasWorldObj()) {
	            ...
	            if (tile != null && !tile.isInvalid()) {
	                ...
	                if (tile.hasCapability(TeslaCapabilities.CAPABILITY_CONSUMER, EnumFacing.UP)) {
	                    ...
	                    if (consumer != null)
	                        this.container.takePower(consumer.givePower(Math.min(this.container.getStoredPower(), ConfigValues.tier1PanelTransferRate), false), false);
	                }
	                    ...
	                    if (consumer != null)
	                        this.container.extractEnergy(consumer.receiveEnergy(ContainerSolarPanelTier1.getIntPower(Math.min(this.container.getStoredPower(), ConfigValues.tier1PanelTransferRate)), false), false);
	                }
	            }
	        }
	    }
