...
public class TileEntityTier5SolarPanel extends TileEntity implements ITickable{

private final ContainerSolarPanelTier5 container;
    
    public TileEntityTier5SolarPanel() {
        
        this.container = new ContainerSolarPanelTier5();
    }
        ...
        if (this.hasWorldObj()) {
            ...
            if (tile != null && !tile.isInvalid()) {
                ...
                if (tile.hasCapability(TeslaCapabilities.CAPABILITY_CONSUMER, EnumFacing.UP)) {
                    ...
                    if (consumer != null)
                        this.container.takePower(consumer.givePower(Math.min(this.container.getStoredPower(), ConfigValues.tier5PanelTransferRate), false), false);
                }
                    ...
                    if (consumer != null)
                        this.container.extractEnergy(consumer.receiveEnergy(ContainerSolarPanelTier5.getIntPower(Math.min(this.container.getStoredPower(), ConfigValues.tier5PanelTransferRate)), false), false);
                }
            }
        }
    }
