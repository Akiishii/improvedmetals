...
public class TileEntityTier10SolarPanel extends TileEntity implements ITickable {

private final ContainerSolarPanelTier10 container;
    
    public TileEntityTier10SolarPanel() {
        
        this.container = new ContainerSolarPanelTier10();
    }
        ...
        if (this.hasWorldObj()) {
            ...
            if (tile != null && !tile.isInvalid()) {
                ...
                if (tile.hasCapability(TeslaCapabilities.CAPABILITY_CONSUMER, EnumFacing.UP)) {
                    ...
                    if (consumer != null)
                        this.container.takePower(consumer.givePower(Math.min(this.container.getStoredPower(), ConfigValues.tier10PanelTransferRate), false), false);
                }
                    ...
                    if (consumer != null)
                        this.container.extractEnergy(consumer.receiveEnergy(ContainerSolarPanelTier10.getIntPower(Math.min(this.container.getStoredPower(), ConfigValues.tier10PanelTransferRate)), false), false);
                }
            }
        }
    }
