...
public class TileEntityTier9SolarPanel extends TileEntity implements ITickable {

private final ContainerSolarPanelTier9 container;
    
    public TileEntityTier9SolarPanel() {
        
        this.container = new ContainerSolarPanelTier9();
    }
        ...
        if (this.hasWorldObj()) {
            ...
            if (tile != null && !tile.isInvalid()) {
                ...
                if (tile.hasCapability(TeslaCapabilities.CAPABILITY_CONSUMER, EnumFacing.UP)) {
                    ...
                    if (consumer != null)
                        this.container.takePower(consumer.givePower(Math.min(this.container.getStoredPower(), ConfigValues.tier9PanelTransferRate), false), false);
                }
                    ...
                    if (consumer != null)
                        this.container.extractEnergy(consumer.receiveEnergy(ContainerSolarPanelTier9.getIntPower(Math.min(this.container.getStoredPower(), ConfigValues.tier9PanelTransferRate)), false), false);
                }
            }
        }
    }
