...
public class TileEntityTier4SolarPanel extends TileEntity implements ITickable{

private final ContainerSolarPanelTier4 container;
    
    public TileEntityTier4SolarPanel() {
        
        this.container = new ContainerSolarPanelTier4();
    }
        ...
        if (this.hasWorldObj()) {
            ...
            if (tile != null && !tile.isInvalid()) {
                ...
                if (tile.hasCapability(TeslaCapabilities.CAPABILITY_CONSUMER, EnumFacing.UP)) {
                    ...
                    if (consumer != null)
                        this.container.takePower(consumer.givePower(Math.min(this.container.getStoredPower(), ConfigValues.tier4PanelTransferRate), false), false);
                }
                    ...
                    if (consumer != null)
                        this.container.extractEnergy(consumer.receiveEnergy(ContainerSolarPanelTier4.getIntPower(Math.min(this.container.getStoredPower(), ConfigValues.tier4PanelTransferRate)), false), false);
                }
            }
        }
    }
