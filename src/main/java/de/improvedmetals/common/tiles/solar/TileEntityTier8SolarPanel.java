...
public class TileEntityTier8SolarPanel extends TileEntity implements ITickable {

private final ContainerSolarPanelTier8 container;
    
    public TileEntityTier8SolarPanel() {
        
        this.container = new ContainerSolarPanelTier8();
    }
        ...
        if (this.hasWorldObj()) {
            ...
            if (tile != null && !tile.isInvalid()) {
                ...
                if (tile.hasCapability(TeslaCapabilities.CAPABILITY_CONSUMER, EnumFacing.UP)) {
                    ...
                    if (consumer != null)
                        this.container.takePower(consumer.givePower(Math.min(this.container.getStoredPower(), ConfigValues.tier8PanelTransferRate), false), false);
                }
                    ...
                    if (consumer != null)
                        this.container.extractEnergy(consumer.receiveEnergy(ContainerSolarPanelTier8.getIntPower(Math.min(this.container.getStoredPower(), ConfigValues.tier8PanelTransferRate)), false), false);
                }
            }
        }
    }
