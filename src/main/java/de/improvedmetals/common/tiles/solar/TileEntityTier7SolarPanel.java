...
public class TileEntityTier7SolarPanel extends TileEntity implements ITickable {

private final ContainerSolarPanelTier7 container;
    
    public TileEntityTier7SolarPanel() {
        
        this.container = new ContainerSolarPanelTier7();
    }
        ...
        if (this.hasWorldObj()) {
            ...
            if (tile != null && !tile.isInvalid()) {
                ...
                if (tile.hasCapability(TeslaCapabilities.CAPABILITY_CONSUMER, EnumFacing.UP)) {
                    ...
                    if (consumer != null)
                        this.container.takePower(consumer.givePower(Math.min(this.container.getStoredPower(), ConfigValues.tier7PanelTransferRate), false), false);
                }
                    ...
                    if (consumer != null)
                        this.container.extractEnergy(consumer.receiveEnergy(ContainerSolarPanelTier7.getIntPower(Math.min(this.container.getStoredPower(), ConfigValues.tier7PanelTransferRate)), false), false);
                }
            }
        }
    }
