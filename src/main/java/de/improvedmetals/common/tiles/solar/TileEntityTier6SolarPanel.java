...
public class TileEntityTier6SolarPanel extends TileEntity implements ITickable {

private final ContainerSolarPanelTier6 container;
    
    public TileEntityTier6SolarPanel() {
        
        this.container = new ContainerSolarPanelTier6();
    }
        ...
        if (this.hasWorldObj()) {
            ...
            if (tile != null && !tile.isInvalid()) {
                ...
                if (tile.hasCapability(TeslaCapabilities.CAPABILITY_CONSUMER, EnumFacing.UP)) {
                    ...
                    if (consumer != null)
                        this.container.takePower(consumer.givePower(Math.min(this.container.getStoredPower(), ConfigValues.tier6PanelTransferRate), false), false);
                }
                    ...
                    if (consumer != null)
                        this.container.extractEnergy(consumer.receiveEnergy(ContainerSolarPanelTier6.getIntPower(Math.min(this.container.getStoredPower(), ConfigValues.tier6PanelTransferRate)), false), false);
                }
            }
        }
    }
