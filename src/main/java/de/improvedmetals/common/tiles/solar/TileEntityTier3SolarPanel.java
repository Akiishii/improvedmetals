...
public class TileEntityTier3SolarPanel extends TileEntity implements ITickable{

private final ContainerSolarPanelTier3 container;
    
    public TileEntityTier3SolarPanel() {
        
        this.container = new ContainerSolarPanelTier3();
    }
        ...
        if (this.hasWorldObj()) {
            ...
            if (tile != null && !tile.isInvalid()) {
                ...
                if (tile.hasCapability(TeslaCapabilities.CAPABILITY_CONSUMER, EnumFacing.UP)) {
                    ...
                    if (consumer != null)
                        this.container.takePower(consumer.givePower(Math.min(this.container.getStoredPower(), ConfigValues.tier3PanelTransferRate), false), false);
                }
                    ...
                    if (consumer != null)
                        this.container.extractEnergy(consumer.receiveEnergy(ContainerSolarPanelTier3.getIntPower(Math.min(this.container.getStoredPower(), ConfigValues.tier3PanelTransferRate)), false), false);
                }
            }
        }
    }
