...
public class TileEntityTier2SolarPanel extends TileEntity implements ITickable{

	private final ContainerSolarPanelTier2 container;
    
    public TileEntityTier2SolarPanel() {
        
        this.container = new ContainerSolarPanelTier2();
    }
        ...
        if (this.hasWorldObj()) {
            ...
            if (tile != null && !tile.isInvalid()) {
                ...
                if (tile.hasCapability(TeslaCapabilities.CAPABILITY_CONSUMER, EnumFacing.UP)) {
                    ...
                    if (consumer != null)
                        this.container.takePower(consumer.givePower(Math.min(this.container.getStoredPower(), ConfigValues.tier2PanelTransferRate), false), false);
                }
                    ...
                    if (consumer != null)
                        this.container.extractEnergy(consumer.receiveEnergy(ContainerSolarPanelTier2.getIntPower(Math.min(this.container.getStoredPower(), ConfigValues.tier2PanelTransferRate)), false), false);
                }
            }
        }
    }
